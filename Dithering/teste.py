...
for b in [False, True]:
  for m in M_l:
    ...
    results_colour.append(co.openner(filename_dec_colour, show_image=True))
...
fig, ax = plt.subplots()
for i in range(2):
  ax.scatter(bpp[i*2:i*2+len(M_l)], mse[i*2:i*2+len(M_l)],c=colors1[i] ,label= "GrayScale {} Dithering".format('with' if i else 'without'))
  ax.scatter(bpp_colour[i*2:i*2+len(M_l)], mse_colour[i*2:i*2+len(M_l)],c=colors2[i] ,label= "Colour {} Dithering".format('with' if i else 'without'))
ax.set_xlabel('bpp')
ax.set_ylabel('MSE')
...
ax.plot(bpp_colour[:v], psnr_colour[:len(M_l)],label="Colour with Dithering")
